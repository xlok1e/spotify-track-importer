...
const runImport = async (state) => {
	// Ensure addedCount exists for states saved before this field was added
	state.addedCount ??= 0;
	...
	if (!state.playlistId) {
		...
		logLine(`Продолжаем: плейлист ${state.playlistId}, уже добавлено ${state.addedCount} треков`);
	}

	const save = () => localStorage.setItem(STORAGE_KEY, JSON.stringify(state));

	/**
	 * Adds all foundUris not yet sent to the playlist.
	 * Updates state.addedCount on success.
	 */
	const flushToPlaylist = async () => {
		const pending = state.foundUris.slice(state.addedCount);
		if (!pending.length) return;
		logLine(`Добавляем ${pending.length} треков в плейлист...`, "info");
		for (let i = 0; i < pending.length; i += 100) {
			...
			for (let attempt = 0; attempt < 3; attempt++) {
				...
				logLine(
					`Ошибка добавления батча: ${res.status} ${err.error?.message ?? "?"} (${attempt + 1}/3)`,
					"err",
				);
				await sleep(2000);
			}
			if (success) {
				state.addedCount += chunk.length;
				save();
			} else {
				logLine(
					`⚠ Часть треков не добавлена (${state.addedCount} из ${state.foundUris.length} добавлено)`,
					"err",
				);
			}
			await sleep(500);
	...
	try {
		for (let i = state.nextIndex; i < state.tracks.length; i += SEARCH_CONCURRENCY) {
			...
			// Flush to playlist every FLUSH_EVERY found tracks
			if (state.foundUris.length - state.addedCount >= FLUSH_EVERY) {
